...
const starts = async (zeroyt7 = new WAConnection()) => {
    ...
    zeroyt7.on('open', () => {
        success('2', 'Terhubung Jangan Lupa Subscribe Fazrin Ezz')
    })
...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(zeroyt7.user.jid)) {
        zeroyt7.sendMessage(anu.jid, "Halo!.. saya WhatsApp BOT ,saya akan membatu mempermudah kehidupan..seperti membuat sticker dan lain-lain. untuk memulai silahkan ketik !menu.", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        sendButImage(mdata.id, wel, "©Created : Fazrin Ezz", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        sendButImage(mdata.id, out, "©Created : Fazrin Ezz", buff, but)
      }
